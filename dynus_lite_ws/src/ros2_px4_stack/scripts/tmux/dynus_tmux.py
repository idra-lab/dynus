...
source_cmd = f"source {workspace_root}/install/setup.bash"
...
if __name__ == "__main__":
    ...
    commands = [
        f"{source_cmd} && source ~/code/decomp_ws/install/setup.bash && ros2 launch {planner} onboard_{planner}.launch.py x:=0.0 y:=0.0 z:=0.0 yaw:=0 namespace:={veh} use_obstacle_tracker:=false use_ground_robot:=false use_hardware:=true " \
        ...
        f"{source_cmd} && sleep 10 && ros2 launch livox_ros_driver2 run_MID360_launch.py namespace:={veh}", # Pane 3

        f"{source_cmd} && source ~/code/dlio_ws/install/setup.bash && sleep 10 && ros2 launch direct_lidar_inertial_odometry dlio.launch.py namespace:={veh}", # Pane 4

        f"sleep 5.0 && ros2 launch mavros px4.launch namespace:={veh}/mavros tgt_system:={mav_id}", # Pane 5

        f"{source_cmd} && sleep 10 && source ~/code/mavros_ws/src/ros2_px4_stack/scripts/trajectories/get_init_pose.sh && ros2 launch ros2_px4_stack dynus_mavros.launch.py odom_type:={odom_type}", # Pane 6
        # f"sleep 30.0 && source {source_cmd} && cd ~/code/data/bags && rm -rf rosbag*", # Pane 7
        # f"sleep 10.0 && source {source_cmd} && cd ~/data && rm -rf * && ros2 bag record -a -o twist_bag", # Pane 7 # /tf /tf_static

        # f"sleep 15 && source /home/swarm/code/decomp_ws/install/setup.bash && rm -rf /home/swarm/data/num_1 && source {source_cmd} && python3 {workspace_root}/src/dynus/scripts/bag_record.py --bag_number 1 --bag_path /home/swarm/data",
        f"sleep 15.0 && source {source_cmd} && cd ~/data && ros2 bag record /trajs {veh}/world {veh}/mavros/local_position/pose {veh}/dlio/odom_node/pose {veh}/mavros/setpoint_trajectory/local {veh}/mavros/imu/data_raw {veh}/mavros/imu/data /tf /tf_static", # Pane 7
        
        f"sleep 15.0 && ros2 topic echo {veh}/mavros/local_position/pose", # Pane 8 

        'sleep 10.0 && source {workspace_root}/src/ros2_px4_stack/scripts/trajectories/get_init_pose.sh && echo && echo "init pos: (${INIT_X}, ${INIT_Y}, ${INIT_Z})" && echo "init att: (${INIT_ROLL}, ${INIT_PITCH}, ${INIT_YAW})" && echo', # Pane 9
